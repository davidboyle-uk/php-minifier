...
class CssParser
{
    ...
    /**
     * {@link CssParserPluginAbstract Plugins}.
     *
     * @var array
     */
     ...
     * @var string
     */
    protected $stateExclusive = false;
     ...
     * @var mixed
     */
    protected $stateMediaTypes = false;
     ...
     * @var array
     */
    protected $states = array("T_DOCUMENT");
     ...
     * Constructer.
     *
     *  Create instances of the used {@link CssParserPluginAbstract plugins}.
     *
    ...
    public function __construct($source = null, array $plugins = null)
    {
        ...
        foreach ($plugins as $name => $config) {
            if ($config !== false) {
                $class  = "PHPMin\CssMin\Plugins\Parsers\Css" . $name . "ParserPlugin";
     ...
     *
     * @param CssTokenAbstract $token Token to append
     * @return void
     */
    public function appendToken(CssTokenAbstract $token)
    {
        $this->tokens[] = $token;
    }
     ...
     * @return CssParserPluginAbstract
     */
    ...
    public function parse($source)
    {
        ...
        for ($i = 0, $l = count($plugins); $i < $l; $i++) {
            ...
            $pluginTriggerStates[$i]        = (
                $tPluginTriggerStates === false ?
                false :
                ...
                if ($exclusive) {
                    ...
                    for ($ii = 0, $ll = $pluginCount; $ii < $ll; $ii++) {
                        ...
                        if (strpos($pluginTriggerChars[$ii], $c) !== false
                            ...
                            if ($r === true) {
                                break;
                            } // Return value is numeric => set new index,
                            //break the plugin loop and and continue with next char
                            elseif ($r !== false && $r != $i) {
                                $i = $r;
                                break;
                            }
    ...
    public function popState()
    {
        ...
        $this->state = isset($this->states[count($this->states) - 1]) ? $this->states[count($this->states) - 1] : null;
        return $r;
    }
     ...
     * equals to {@link CssParser::popState()} + {@link CssParser::pushState()}.
     *
