 ...
 * for parsing @page at-rule block with including declarations.
 *
 * Found @page at-rule blocks will add a {@link CssAtPageStartToken}
 * and {@link CssAtPageEndToken} to the
 ...
 * @version     1.0.1
 */
class CssAtPageParserPlugin extends CssParserPluginAbstract
{
    /**
    ...
    public function parse($index, $char, $previousChar, $state)
    {
        ...
        if ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(
                substr($this->parser->getSource(), $index, 5)
            ) === "@page"
        ) {
        ...
        elseif (($char === ";" || $char === "}")
            ...
            $this->parser->appendToken(
                new CssAtPageDeclarationToken($this->buffer, $value, $isImportant)
            );
            // --
            if ($char === "}") {
                $this->parser->popState();
