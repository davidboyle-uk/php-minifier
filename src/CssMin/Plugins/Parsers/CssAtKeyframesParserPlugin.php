...
/**
 * {@link CssParserPluginAbstract Parser plugin}
 * for parsing @keyframes at-rule blocks, rulesets and declarations.
 *
 ...
 * @version     1.0.1
 */
class CssAtKeyframesParserPlugin extends CssParserPluginAbstract
{
    /**
     * @var string Keyword
     */
    protected $atRuleName = "";
    ...
    public function parse($index, $char, $previousChar, $state)
    {
        ...
        elseif ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(
                substr($this->parser->getSource(), $index, 15)
            ) === "@-moz-keyframes"
        ) {
        ...
        elseif ($char === "@"
            && $state === "T_DOCUMENT"
            && strtolower(
                substr($this->parser->getSource(), $index, 18)
            ) === "@-webkit-keyframes"
        ) {
        ...
        elseif ($char === "{" && $state === "T_AT_KEYFRAMES_RULESETS") {
            if ($this->parser->getBuffer() !== "") {
                ...
                $this->parser->appendToken(
                    new CssAtKeyframesRulesetStartToken($this->selectors)
                );
        ...
        elseif (($char === ";" || $char === "}")
            ...
            $this->parser->appendToken(
                new CssAtKeyframesRulesetDeclarationToken(
                    $this->buffer,
                    $value,
                    $isImportant
                )
